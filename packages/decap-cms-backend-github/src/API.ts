...
export default class API {
  ...
  static DEFAULT_COMMIT_MESSAGE = 'Automatically generated by Startr Web-Admin';
  ...
  checkMetadataRef() {
    return this.request(`${this.repoURL}/git/refs/meta/_decap_cms`)
      ...
      .catch(() => {
        ...
        const readme = {
          raw: '# Startr Web-Admin\n\nThis tree is used by the Startr Web-Admin to store metadata information for specific files and branches.',
        };
    ...
    if (this.useOpenAuthoring) {
      ...
      for (const pr of pullRequests) {
        if (!migrationNotified) {
          ...
          alert(oneLine`
            Startr Web-Admin is adding labels to ${pullRequests.length} of your Editorial Workflow
  ...
  async forceMergePR(pullRequest: GitHubPull) {
    ...
    let commitMessage = 'Automatically generated. Merged on Startr Web-Admin\n\nForce merge of:';
    files.forEach(file => {
      commitMessage += `\n* "${file.path}"`;
    });
    console.log(
